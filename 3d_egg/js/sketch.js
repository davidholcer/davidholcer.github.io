...
var shell;
// let rotationFactor;
var paused,spotted,visible,lines;
...
function setup() {
  ...
  sliderRange(-200, 200, -1);
  gui.addGlobals('rotationFactor');
  // w_l=random();
  // S=random();
...
function draw() {
  ...
  for(let i=0;i<angleTotal;i++){
    ...
    for(let j=0;j<=total;j++){
        ...
        //brown egg
        if (brown){
          let hue=map(v1.z,-L/2,L/2,21,35);
          let saturation=map(v1.z,-L/2,L/2,69,90);
          let brightness=map(v1.x,-L/2,L/2,60,100);
          fill(hue,saturation,brightness);  
        }
        
        //white egg
        else{
          let hue=map(v1.z,-L/2,L/2,38,30);
          let saturation=map(v1.z,-L/2,L/2,31,30);
          let brightness=map(v1.x,-L/2,L/2,80,100);
          fill(hue,saturation,brightness);  
        }
   ...
   for(let i=0;i<angleTotal;i++){
    for(let j=0;j<=total;j++){
        ...
        if (pLines<lineDensity){
          push()
          rotateZ(playedCount/500*rotationFactor);
            if (brown){
            let hue=map(rV1.z,-L/2,L/2,21,35);
            let saturation=map(rV1.z,-L/2,L/2,69,90);
            let brightness=map(rV1.z,-L/2,L/2,80,100);
            stroke(hue,saturation,brightness);  
          }
        
          //white egg
          else{
            let hue=map(rV1.z,-L/2,L/2,38,30);
            let saturation=map(rV1.z,-L/2,L/2,31,30);
            let brightness=map(rV1.z,-L/2,L/2,80,100);
            stroke(hue,saturation,brightness);  
          }
...
// function afterRotation(x,y,z,t){
//     let Pn=[[x],[y],[z]]
//     // t=2*PI*i/angleTotal;
//     let Ttn=T(t);
//     let Ppn=multiplyMatrices(Ttn,Pn);
//     return Pn;
//     // return([Ppn[0],Ppn[1]]);
// }
